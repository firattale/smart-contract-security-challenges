...
describe("Reentrancy Exercise 4", function () {
    ...
    before(async () => {
        ...
        cryptoEmpireGame = await CryptoEmpireGameFactory.deploy(
            cryptoEmpireToken.address
        );
    ...
    after(async () => {
        ...
        expect(
            await cryptoEmpireToken.balanceOf(attacker.address, 2)
        ).to.be.at.least(20);
        expect(
            await cryptoEmpireToken.balanceOf(cryptoEmpireGame.address, 2)
        ).to.be.equal(0);
    });
});
